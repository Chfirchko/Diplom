...
class Human:
    def __init__(self, start, goal, queue):
...
for i in range(2):
    human = Human((0, i), (0, i), [])
    heappush(human.queue, (0, human.start))
...
while True:
    ...
    for agent in Humanity:
        ...
        if mouse_pos:
            agent.visited = dijkstra(agent.start, mouse_pos, graph)
        ...
        pg.draw.circle(sc, pg.Color('green'), *get_circle(*agent.start))
